...
function talkform($section,$talkline,$limit=20,$schema=false){
	...
	if ($displaymode==0) {
		...
		foreach($colors as $key=>$val) {
			$break++;
			rawoutput("<a class='$val' href='#colors' onClick='addColor(\"`$key\", \"inputinsertcommentary\");'>$key</a>");
			if ($break==30) {
				rawoutput("<br>");
				$break=0;
			}
	...
	if(get_module_pref("user_textarea","textboxsetting")) {
		previewfield("insertcommentary", $session['user']['name'], $talkline, true, array("type"=>"textarea", "cols"=>"120", "rows"=>"4", "maxlength"=>500-$tll),false,false,$writingcharlist);
	} else {
		previewfield("insertcommentary", $session['user']['name'], $talkline, true, array("size"=>"60", "maxlength"=>500-$tll),false,false,$writingcharlist);
	}
	...
	if ($displaymode==1) {
		...
		//Eigener Name
		rawoutput("<a shref='#colors' onClick='addColor(\"{$session['user']['name']}\", \"inputinsertcommentary\");'>");
		output_notl($session['user']['name']);
		rawoutput("</a><br>");
	}
