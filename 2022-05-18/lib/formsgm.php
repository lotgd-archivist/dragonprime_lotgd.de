...
function previewfieldcouncil($name, $startdiv=false, $talkline="says", $charsleft=true, $info=false, $allowspecial=false, $default=false,$npcoutput=false) {
	showpreviewfield($name,$startdiv,$talkline,$charsleft,$info,$allowspecial,$default,$npcoutput);
}


function previewfieldgm($name, $startdiv=false, $talkline="says", $charsleft=true, $info=false, $allowspecial=false, $default=false) {
	$npclist=unserialize(get_module_setting("othernpcs","gamemaster"));
	showpreviewfield($name,$startdiv,$talkline,$charsleft,$info,$allowspecial,$default,$npclist);
}


function showpreviewfield($name, $startdiv=false, $talkline="says", $charsleft=true, $info=false, $allowspecial=false, $default=false, $npcoutput=false) {
	...
	rawoutput("<script language='JavaScript'>
				...
				var npcidx = document.getElementById('inputnpc" . $name . "').selectedIndex;
				var npcvalue = document.getElementById('inputnpc" . $name . "').value;
				var npcname = document.getElementById('inputnpc" . $name . "').options[npcidx].text;
				var npc = new Array();");
	$i=0;
	foreach ($npcoutput as $id=>$npcname) {
...
//									}
	...
	rawoutput("				}
				...
				");
	if (!is_array($info)) {
		rawoutput("<input name='$name' id='input$name' maxlength='500' onKeyUp='previewtext$name(document.getElementById(\"input$name\").value,500);'>");
	} else {
